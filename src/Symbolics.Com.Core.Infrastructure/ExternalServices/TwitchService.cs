...
public sealed class TwitchService : ITwitchService
{
    ...
    public Task<TwitchStreamResponse> GetStreams(string? cursor)
    {
        ...
        _logger.LogInformation("Fetching Twitch streams (placeholder). BaseUrl: {BaseUrl}, Cursor: {Cursor}", options.BaseUrl, cursor ?? "<none>");
    ...
    {
        var options = _optionsMonitor.CurrentValue;
        _logger.LogInformation("Fetching Twitch streamer info (placeholder). BaseUrl: {BaseUrl}, Login: {Login}", options.BaseUrl, twitchLogin);
    ...
    {
        var options = _optionsMonitor.CurrentValue;
        _logger.LogInformation("Fetching Twitch game info (placeholder). BaseUrl: {BaseUrl}, GameId: {GameId}", options.BaseUrl, twitchGameId);
